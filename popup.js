...
const getStatus = async () => {
  ...
  return chrome.runtime.sendMessage({ type: "get-status", tabId: tab?.id });
};

const toggleSite = async () => {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  const state = await chrome.runtime.sendMessage({ type: "toggle-site", tabId: tab?.id });
  render(state);
};

const toggleAlwaysOn = async () => {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  const state = await chrome.runtime.sendMessage({ type: "toggle-always", tabId: tab?.id });
  render(state);
};
