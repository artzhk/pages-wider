...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  handleMessage().then(sendResponse).catch(() => sendResponse(null));
  return true;
});

const getStateForTab = async (tabId) => {
  if (!tabId) {
    return { alwaysOn: false, siteEnabled: false, domain: null, isHttp: false, savedSites: [], settings: DEFAULT_SETTINGS };
  }
